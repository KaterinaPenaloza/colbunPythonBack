...
const ChatComponent = () => {
      ...
      const anotherQuestionMessage = {
        bot:
          <div>
            ...
            <button className="opciones-buttons" onClick={handleReturnToQuestions}>
    Volver a preguntas
  </button>
  ...
  const handleThemeSelection = (theme) => {
    setSelectedTheme(theme); // Actualiza el tema seleccionado
    setQuestions(questionSets[theme]); // Actualiza las preguntas para el tema

    // Actualiza el historial del chat con las preguntas del tema
    const questionButtons = questionSets[theme].map((question, index) => ({
      bot: (
        <div key={index}>
  ...
  const handleSubThemeSelection = (theme, subtema) => {
    // Validaciones
    if (!themesAndQuestions[theme] || !themesAndQuestions[theme].subtemas[subtema]) {
      console.error(`Tema o subtema no encontrado: ${theme} - ${subtema}`);
      return;
    }

    setSelectedTheme(theme);
    setSelectedSubTheme(subtema);
    console.log("Subtema seleccionado:", subtema);

    // Obtener las preguntas del subtema
    const preguntas = themesAndQuestions[theme].subtemas[subtema].preguntas || [];
    const subtematitulo = themesAndQuestions[theme].subtemas[subtema].titulo;
    ...
    setChatHistory([
      ...
      ...questionButtons,
      {
        bot: (
          <div>
            <button className="opciones-buttons" onClick={() => handleSubThemeSelection(theme, subtema)}>
              Volver a preguntas
            </button>
            <button
              className="opciones-buttons"
              onClick={() => handleThemeSelection(theme)} // Volver a los subtemas
            >
              Volver a subtemas
            </button>
            <button
              className="opciones-buttons"
              onClick={() => handleThemeList()} // Volver a los temas
            >
              Volver a temas
            </button>
          </div>
  ...
  const handleReturnToQuestions = () => {
    if (selectedTheme && selectedSubTheme) {
      handleSubThemeSelection(selectedTheme, selectedSubTheme);
    }
  };
  ...
  useEffect(() => {
    document.title = 'Chatbot Instructivos Colbun';
        ...
        <ul className="chat-history">
          ...
          {loading && (
            <li className="message bot-message loading-message">
              <PulseLoader size={12} className="custom-loader" />
            </li>
          )}
        </ul>
      </div>
